...
class hardModeViewController: UIViewController {
    ...
    var highScor: Int = 0
    var timeOfProgressView : Float = 5
    ...
    // MARK: - Easy Mode 05 Second Timing Setting
    
    func setTimerForProgressView(){
        timeOfProgressView = 5
    ...
    @objc func updateTimer(){
        if timeOfProgressView < 0{
            ...
            progressView.progress = Float(timeOfProgressView)/5
        }
    }
    ...
    func replayTapped(){
        let replayButton = navigationController?.storyboard?.instantiateViewController(withIdentifier: "hardModeViewController") as! hardModeViewController
    ...
    // MARK: - Action when Answer Button Tapped
